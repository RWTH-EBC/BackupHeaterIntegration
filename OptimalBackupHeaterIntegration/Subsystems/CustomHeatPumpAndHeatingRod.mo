...
model CustomHeatPumpAndHeatingRod
  "Custom Bivalent monoenergetic heat pump"
  ...
  Modelica.Blocks.MathBoolean.Or
                             or1(nu=if use_heaRod then 3 else 2)
  ...
  Modelica.Blocks.Logical.Hysteresis       isOnHP(uLow=0.001, uHigh=0.005)
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=270,
        origin={16,38})));
  ...
  Modelica.Blocks.Sources.BooleanConstant
    ...
    annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-164,56})));
  Modelica.Blocks.Logical.Hysteresis       isOnHR(uLow=0.0001, uHigh=0.0005)
    if use_heaRod
    annotation (Placement(transformation(
        extent={{-6,-6},{6,6}},
        rotation=270,
  ...
  IBPSA.Fluid.Movers.SpeedControlled_y pump(
    ...
    final y_start=1) if use_pressure annotation (Placement(transformation(
        ...
        rotation=180,
        origin={16,-68})));
  BESMod.Components.HeatingRodWithSecurityControl hea(
    redeclare package Medium = Medium,
    final allowFlowReversal=allowFlowReversal,
    final m_flow_nominal=m_flow_nominal[1],
    ...
    m_flowTurnOff=hea.m_flow_nominal*0.1,
    m_flowTurnOn=hea.m_flow_nominal*0.15)
                           if use_heaRod
...
BESMod.Utilities.KPIs.InternalKPICalculator KPIQHR(
  ...
  BESMod.Utilities.Electrical.ZeroLoad zeroLoad annotation (Placement(
        ...
        rotation=180,
        origin={100,-78})));
  BESMod.Components.DiscretizeContSignal discretizeContSignal(
      discretizationSteps=heatingRodParameters.discretizationSteps)
    if use_heaRod
    annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
  BESMod.Utilities.KPIs.InputKPICalculator KPIWelPump(
    integralUnit="J",
    calc_singleOnTime=true,
    calc_integral=true,
    calc_movAve=false,
    unit="W")
    annotation (Placement(transformation(extent={{-96,-128},{-84,-106}})));
protected
...
connect(KPIQHR.KPIBus, outBusGen.QHR_flow) annotation (Line(points={{-63.88,-131},{0,-131},{0,-100}}, color={255,204,51}, thickness=0.5), Text(string="%second", index=1, extent={{6,3},{6,3}}, horizontalAlignment=TextAlignment.Left));
  ...
  connect(KPIWHRel.KPIBus, outBusGen.WHRel) annotation (Line(
      ...
      horizontalAlignment=TextAlignment.Left));
  if use_heaRod then
    connect(isOnHR.y, or1.u[3]) annotation (Line(points={{40,29.4},{40,26},{46,26},
          {46,20}},   color={255,0,255}));
  end if;
  connect(isOnHP.y, or1.u[1]) annotation (Line(points={{16,31.4},{16,24},{46,24},
          {46,20}}, color={255,0,255}));
  connect(constRunPump247.y, or1.u[2]) annotation (Line(points={{73.5,27},{48.1,
          27},{48.1,20},{46,20}},   color={255,0,255}));
  ...
  connect(zeroLoad.internalElectricalPin, internalElectricalPin) annotation (
      Line(
      points={{90,-78},{72,-78},{72,-100}},
      color={0,0,0},
      thickness=1));

  connect(discretizeContSignal.y, hea.u) annotation (Line(points={{-39,70},{-12,
          70},{-12,72},{12,72},{12,89.6},{18.8,89.6}}, color={0,0,127}));
  connect(discretizeContSignal.u, sigBusGen.hr_on) annotation (Line(points={{
          -62,70},{-68,70},{-68,98},{2,98}}, color={0,0,127}), Text(
      string="%second",
      index=1,
      extent={{-6,3},{-6,3}},
      horizontalAlignment=TextAlignment.Right));
  connect(pump.P, KPIWelPump.u) annotation (Line(points={{5,-59},{-104,-59},{
          -104,-60},{-144,-60},{-144,-117},{-97.32,-117}}, color={0,0,127}));
  connect(KPIWelPump.KPIBus, outBusGen.WPumpel) annotation (Line(
      points={{-83.88,-117},{0,-117},{0,-100}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%second",
      index=1,
      extent={{6,3},{6,3}},
      horizontalAlignment=TextAlignment.Left));
end CustomHeatPumpAndHeatingRod;
